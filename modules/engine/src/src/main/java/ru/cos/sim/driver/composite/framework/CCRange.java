...
public abstract class CCRange implements Cloneable{
	...
	protected Map<Criterion.CriterionType, Criterion> criteria = new HashMap<Criterion.CriterionType, Criterion>();

	public CCRange() {
		criteria.put(Criterion.CriterionType.Priority, new PriorityCriterion(Priority.Lowest));
		criteria.put(Criterion.CriterionType.Acceleration, new AccelerationCriterion(-Float.MAX_VALUE));
	}
	...
	public static CCRange calculateResultantRange(CCRange ccRange1, CCRange ccRange2){
		...
		if (resultantCCRange.isEmpty()){
			for (Criterion.CriterionType criterionType: Criterion.CriterionType.values()){
	 ...
	 */
	public Criterion getCriterion(Criterion.CriterionType criterionType){
		return criteria.get(criterionType);
	}
	
	public void setPriority(Priority priority){
		((PriorityCriterion)criteria.get(Criterion.CriterionType.Priority)).setPriority(priority);
	}
	...
	public Priority getPriority() {
		return ((PriorityCriterion)criteria.get(Criterion.CriterionType.Priority)).getPriority();
	}
}
