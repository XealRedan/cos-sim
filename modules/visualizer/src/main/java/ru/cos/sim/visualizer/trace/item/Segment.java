...
public class Segment extends StaticEntity {
	...
	protected Link link;
	protected ru.cos.sim.visualizer.traffic.parser.trace.Segment.SegmentType segmentType;
	...
	{
		this.segmentType = ru.cos.sim.visualizer.traffic.parser.trace.Segment.SegmentType.Narrow;
		acceptLanes();
		if (!this.geometry.leftSide.overrided && !this.geometry.rightSide.overrided){
			...
			this.segmentType = ru.cos.sim.visualizer.traffic.parser.trace.Segment.SegmentType.Simple;
			return true;
		} return false;
	...
	@Override
	public void draw(Renderer.RenderType mode) {
		if (this.form.getLastFrustrumState() == IRenderable.FrustrumState.OutOfView){
	...
	public void checkFrustum()
	{
		...
		if (this.frustrumState == IRenderable.FrustrumState.OutOfView){
			FrameDataHandler.getInstance().changeObjectToInvisible(frameInfo);
		} else {
			FrameDataHandler.getInstance().changeObjecttoVisible(frameInfo);
		}
	...
	public ru.cos.sim.visualizer.traffic.parser.trace.Segment.SegmentType getSegmentType()
	{
