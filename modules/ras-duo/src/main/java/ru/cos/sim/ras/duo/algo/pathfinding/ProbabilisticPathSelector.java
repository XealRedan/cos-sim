...
import ru.cos.sim.ras.duo.dijkstra.DijkstraVertexInfo;

public class ProbabilisticPathSelector implements DijkstraProcessor.PathSelector {

	public ProbabilisticPathSelector(float temperature) {
		this(new ProbabilisticSelector<DijkstraVertexInfo.Backtrack>(temperature));
	}

	public ProbabilisticPathSelector(ProbabilisticSelector<DijkstraVertexInfo.Backtrack> selector) {
		this.selector = selector;
	}
	
	private ProbabilisticSelector<DijkstraVertexInfo.Backtrack> selector;
	
	@Override
	public DijkstraVertexInfo.Backtrack select(SortedList<Backtrack> variants) {
		for (DijkstraVertexInfo.Backtrack variant : variants)
			selector.addVariant(variant.getTotalWeight(), variant);
			
		DijkstraVertexInfo.Backtrack selectedVariant = selector.select();
		selector.clear();
		return selectedVariant;
	}
	
	public static class UniformFactory implements Factory {
		...
		@Override
		public DijkstraProcessor.PathSelector createPathSelector(DijkstraProcessor processor, Vertex start, Vertex end) {
			return new ProbabilisticPathSelector(temperature);
		}
	}
	
	public static class BestPathSpeedCongestionFactory implements Factory {
		...
		public DijkstraProcessor.PathSelector createPathSelector(DijkstraProcessor processor, Vertex start, Vertex end) {
			...
			LinkedList<DijkstraVertexInfo.Backtrack> bestPath = processor.collectPath(start, end, new BestOnePathSelector.Factory());
